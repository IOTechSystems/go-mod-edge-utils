...
func (c *Client) GetSecret(secretName string, keys ...string) (map[string]string, error) {
	...
	if len(notFound) > 0 {
		return nil, secrets.NewErrSecretsNotFound(notFound)
	}
...
func (c *Client) getTokenDetails() (*types.TokenMetadata, error) {
	...
	if jsonDec == nil {
		return nil, secrets.NewErrSecretStore("failed to obtain json decoder")
	}
...
func (c *Client) refreshToken(ctx context.Context, tokenExpiredCallback secrets.TokenExpiredCallback) error {
...
func (c *Client) getSecretData(secretName string) (map[string]string, error) {
	...
	if resp.StatusCode == 404 {
		return nil, secrets.NewErrSecretNameNotFound(fmt.Sprintf("Received a '%d' response from the secret store", resp.StatusCode))
	}
...
func (c *Client) store(secretName string, secretsMap map[string]string) error {
	if len(secretsMap) == 0 {
		// nothing to store
		return nil
	}
	...
	payload, err := json.Marshal(secretsMap)
	if err != nil {
		return err
	}
	...
	if resp.StatusCode < 200 || resp.StatusCode > 299 {
		return secrets.NewErrSecretStore(fmt.Sprintf("Received a '%d' response from the secret store", resp.StatusCode))
	}
...
func (c *Client) IsJWTValid(jwt string) (bool, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode > 299 {
		return false, secrets.NewErrSecretStore(fmt.Sprintf("Received a '%d' response from the secret store", resp.StatusCode))
	}
