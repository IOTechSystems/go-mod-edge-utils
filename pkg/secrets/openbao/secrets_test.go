...
func TestMultipleTokenRenewals(t *testing.T) {
	...
	tests := []struct {
		name                     string
		authToken                string
		tokenExpiredCallbackFunc secrets.TokenExpiredCallback
		expectError              bool
...
func TestHttpSecretStoreManager_GetSecret(t *testing.T) {
	TestConnError := secrets.NewErrSecretStore("testing conn error")
	TestConnErrorSecretNameNotFound := secrets.NewErrSecretNameNotFound("testing secretName error")
	...
	tests := []struct {
		...
		},
		{
			...
			expectedValues:    nil,
			expectedErrorType: secrets.NewErrSecretsNotFound([]string{"Does not exist"}),
			expectedDoCallNum: 1,
			caller: &InMemoryMockCaller{
				Data: testData,
			},
		},
		{
			...
			expectedValues:    nil,
			expectedErrorType: secrets.NewErrSecretsNotFound([]string{"Does not exist", "Also does not exist"}),
			expectedDoCallNum: 1,
			caller: &InMemoryMockCaller{
				Data: testData,
			},
		...
		},
		{
			...
			caller: &ErrorMockCaller{
				ReturnError: false,
				StatusCode:  404,
				ErrorType:   secrets.NewErrSecretNameNotFound("Not found"),
			},
		},
		{
			...
			caller: &ErrorMockCaller{
				ReturnError: false,
				StatusCode:  400,
				ErrorType:   secrets.NewErrSecretStore("Error"),
			},
		},
		{
...
func (smhc *SimpleMockAuthHttpCaller) Do(req *http.Request) (*http.Response, error) {
	...
	case http.MethodGet, http.MethodPost:
		if smhc.returnError {
			...
			}, secrets.NewErrSecretStore("http response error")
		}
...
func TestHttpSecretStoreManager_GetSecretNames(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			basePath:          "one",
			expectedValues:    nil,
			expectedErrorType: secrets.NewErrSecretNameNotFound("Does not exist"),
			expectedDoCallNum: 1,
			caller: &ErrorMockCaller{
				ReturnError: false,
				StatusCode:  404,
				ErrorType:   secrets.NewErrSecretNameNotFound("Does not exist"),
			},
		},
		{
			name:              "one name",
