...
func VaultAuthenticationHandlerFunc(secretProvider interfaces.SecretProviderExt, logger log.Logger) echo.MiddlewareFunc {
	return func(inner echo.HandlerFunc) echo.HandlerFunc {
		return func(c echo.Context) error {
			...
			logger.Debugf("Authorizing incoming call to '%s' via JWT (Authorization len=%d)", r.URL.Path, len(authHeader))
			authParts := strings.Split(authHeader, " ")
			if len(authParts) >= 2 && strings.EqualFold(authParts[0], "Bearer") {
				token := authParts[1]
				validToken, err := secretProvider.IsJWTValid(token)
				if err != nil {
					logger.Errorf("Error checking JWT validity: %v", err)
					// set Response.Committed to true in order to rewrite the status code
					w.Committed = false
					return echo.NewHTTPError(http.StatusInternalServerError, http.StatusText(http.StatusInternalServerError))
				} else if !validToken {
					logger.Warnf("Request to '%s' UNAUTHORIZED", r.URL.Path)
					// set Response.Committed to true in order to rewrite the status code
					w.Committed = false
					return echo.NewHTTPError(http.StatusUnauthorized, http.StatusText(http.StatusUnauthorized))
				}
				logger.Debugf("Request to '%s' authorized", r.URL.Path)
				return inner(c)
			}
...
func AutoConfigAuthenticationFunc(secretProvider interfaces.SecretProviderExt, logger log.Logger) echo.MiddlewareFunc {
	...
	if secret.IsSecurityEnabled() && !disableJWTValidation {
		authenticationHook = VaultAuthenticationHandlerFunc(secretProvider, logger)
	}
