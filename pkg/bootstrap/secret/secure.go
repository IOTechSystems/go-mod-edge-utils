...
func (p *SecureProvider) SetClient(client client.SecretClient) {
...
func (p *SecureProvider) seedSecrets(contents []byte) ([]byte, error) {
	...
	for index, secret := range serviceSecrets.Secrets {
		...
		serviceSecrets.Secrets[index].SecretData = make([]models.SecretDataKeyValue, 0)
	}
...
func (p *SecureProvider) HasSecret(secretName string) (bool, error) {
	...
	if err != nil {
		_, ok := err.(secrets.ErrSecretNameNotFound)
		if ok {
			return false, nil
		}
